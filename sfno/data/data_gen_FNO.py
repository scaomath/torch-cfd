...
def main(args):
    ...
    >>> python data_gen_FNO.py --sample-size 1280 --batch-size 256 --grid-size 256 --subsample 4 --extra-vars --time 50 --time-warmup 30 --num-steps 100 --dt 1e-3 --visc 1e-3

    - Test data
    >>> python data_gen_FNO.py --sample-size 16 --batch-size 8 --grid-size 256 --subsample 1 --double --extra-vars --time 50 --time-warmup 30 --num-steps 100 --dt 1e-3 --replicable-init --seed 42

    - Test data fine
    >>> python data_gen_FNO.py --sample-size 2 --batch-size 1 --grid-size 512 --subsample 1 --double --extra-vars --time 50 --time-warmup 30 --num-steps 200 --dt 5e-4 --replicable-init --seed 42
    ...
    solver_kws = dict(
        visc=visc, delta_t=delta_t, diam=diam, dealias=dealias, dtype=torch.float64
    )
    ...
    for i, idx in enumerate(range(0, N_samples, batch_size)):
        logger.info(f"Generate trajectory for {i+1}-th batch of {N_samples} samples")
        logger.info(
            f"random state: {args.seed + idx} to {args.seed + idx + batch_size-1}"
        )
